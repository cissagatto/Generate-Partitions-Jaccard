...
#       correlation matrix                                                                       #
##################################################################################################
computeJaccard <- function(ds, dataset_name, number_folds, namesLabels, resLS, folderResults){
  ...
  diretorios = directories(dataset_name, folderResults)
    ...
    jaccardParalel <- foreach(s = 1:number_folds) %dopar%{
      ...
      FolderHCES = paste(diretorios$folderResultsJaccard, "/Split-", s, sep="")
...
##################################################################################################
CutreeHClust <- function(ds, dataset_name, number_folds, namesLabels, resLS, folderResults){
  ...
  cutreeParalel <- foreach(s = 1:number_folds) %dopar%{
    ...
    resumePartitions = data.frame(fold, partition, num.groups)
      ...
      #cat("\nOTTER DENDRO\n")                  
      OtterDendro = as.dendrogram(hclust(d = as.dist(matrix_correlation_2), method="single"))
